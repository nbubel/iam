...
var iam = ( function(parentmodule) {
		...
		console.log("loading indexeddb as submodule lib.indexeddb of: " + parentmodule);
		...
		function IndexDBCRUDOperationsImpl(_dbname, _version, _objectstores, _modifiers, _onstorecreated, _intids) {
			...
			for (var i = 0; i < _objectstores.length; i++) {
				if (modifiers[i] && modifiers[i].keyPath) {
			...
			this.createObject = function(objectstore, object, onsuccess, onerror, context) {
				...
				transaction.oncomplete = function(event) {
					console.log("createObject(): oncomplete. got id: " + event.target.result + " (note that the id will only be set if it is assigned by the db, i.e. undefined is ok for manually assigned ids)");
